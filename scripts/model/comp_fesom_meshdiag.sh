#!/bin/bash
# Build FESOM's standalone mesh-diagnostics utility (fesom_meshdiag) for writing
# fesom.mesh.diag.nc.  Derived from the generated comp-fesom_dp_script.sh: same modules and
# cmake flags (double precision), plus -DBUILD_MESHDIAG=ON, in a separate build_meshdiag/
# directory, and only the fesom_meshdiag target: no 'make install', so build/, lib64/ and
# bin/ (what runs stage from) are not touched.
#!/bin/bash -l
...
cd /work/ab0246/a270092/model_codes/awiesm3-develop
pushd fesom-2.7
export XIOS_ROOT=/work/ab0246/a270092/model_codes/awiesm3-develop/fesom-2.7/../xios; mkdir -p build_meshdiag; cd build_meshdiag; cmake -DBUILD_MESHDIAG=ON -DOIFS_COUPLED=ON -DFESOM_COUPLED=ON -DENABLE_OPENMP=ON -DOPENMP_REPRODUCIBLE=OFF -DDISABLE_MULTITHREADING=OFF -DDISABLE_PARALLEL_RESTART_READ=ON  -DFESOM_WITH_XIOS=ON -DCMAKE_BUILD_TYPE=Release -DDISABLE_MULTITHREADING=OFF    -DCMAKE_INSTALL_PREFIX=../  ..;   make -j 16 fesom_meshdiag
popd
